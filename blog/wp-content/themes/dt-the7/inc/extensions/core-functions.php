...
function dt_get_resized_img( $img, $opts ) {
	...
	if ( dt_retina_on() && isset($_COOKIE['devicePixelRatio']) && $_COOKIE['devicePixelRatio'] != 1 ) { $img_h *= 2; $img_w *= 2; }

	$file_url = aq_resize( $img[0], $img_w, $img_h, true, true, true );
...
function dt_get_thumb_img( $opts = array() ) {
	...
	// proportion
	if ( $original_image && !empty($opts['prop']) ) {
		$_prop = $opts['prop'];
	...
	if ( $img_id = absint( $opts['img_id'] ) ) {
		if ( !$opts['alt'] ) $opts['alt'] = get_post_meta( $img_id, '_wp_attachment_image_alt', true );
		if ( !$opts['img_title'] ) $opts['img_title'] = get_the_title( $img_id );
	}
	...
	$output = str_replace(
		array( '%HREF%', '%CLASS%', '%TITLE%', '%CUSTOM%', '%SRC%', '%IMG_CLASS%', '%SIZE%', '%ALT%', '%IMG_TITLE%' ),
		array(
			'href="' . esc_url( $href ) . '"',
			$class,
			$title,
...
function dt_get_retina_sensible_image ( $logo, $r_logo, $default, $custom = '', $class = '' ) {
	...
	if ( dt_retina_on() ) {
		$img_meta = (isset($_COOKIE['devicePixelRatio']) && $_COOKIE['devicePixelRatio'] != 1) ? $r_logo : $logo;
	} else {
		$img_meta = $logo;
	}
...
function dt_is_hd_device() {
	return (isset($_COOKIE['devicePixelRatio']) && $_COOKIE['devicePixelRatio'] != 1);
}
