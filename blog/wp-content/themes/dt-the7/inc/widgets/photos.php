	...
	function widget( $args, $instance ) {
		...
		if ( $a_query && $a_query->have_posts() ) {
			...
			while ( $a_query->have_posts() ) { $a_query->the_post();
				...
				$share_buttons = presscore_get_share_buttons_for_prettyphoto( 'photo', array( 'id' => $post_id ) );

				$thumb_meta = wp_get_attachment_image_src( $post_id, 'full' );
				$img = dt_get_thumb_img( array(
					'img_meta'      => $thumb_meta,
					'img_id'		=> $post_id,
					'class'			=> 'rollover rollover-small',
					'custom'		=> 'data-pp="prettyPhoto[widget-photos-' . $number . ']" ' . $share_buttons,
					'options'       => array( 'w' => 90, 'h' => 90 ),
					'wrap'          => "\n" . '<a %HREF% %TITLE% %CLASS% %CUSTOM%><img %IMG_CLASS% %SRC% %SIZE% %ALT% /></a>' . "\n",
				) );
			}
			wp_reset_postdata();
