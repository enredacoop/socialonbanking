...
if ( ! function_exists( 'presscore_get_fullwidth_slider_two' ) ) :
	...
	function presscore_get_fullwidth_slider_two( $attachments_data, $options = array() ) {
		...
		$fields_white_list = array( 'arrows', 'title', 'meta', 'description', 'link', 'details' );
		...
		$show_arrows = true; //in_array( 'arrows', $options['fields'] );
		$show_content = array_intersect( $options['fields'], array('title', 'meta', 'description', 'link', 'details') ) && 'page' == $link;
		...
		foreach ( $attachments_data as $data ) {
			...
			/*if ( $show_content || 'file' == $link || 'none' == $link ) {
				...
				$html .= "\n\t" . '<li class="fs-entry ts-cell" data-dt-link="' . esc_url($data['permalink']) . '"><div class="fs-entry-slide">';
			}
			...
			switch( $link ) {
				...
				case 'file':
					// add anchor to image
					$img_args['wrap'] = sprintf( '<a href="%s" data-pp="prettyPhoto" %%TITLE%%>%s</a>', $data['full'], $img_base_args['wrap'] );
				default:
					$html .= '<div class="fs-entry-img">';
			}
...
if ( ! function_exists( 'presscore_get_images_gallery_hoovered' ) ) :
	...
	function presscore_get_images_gallery_hoovered( $attachments_data, $options = array() ) {
		...
		$big_image_args = array(
			'img_meta' 	=> array( $big_image['full'], $big_image['width'], $big_image['height'] ),
			'img_id'	=> empty( $big_image['ID'] ) ? $big_image['ID'] : 0,
			'wrap'		=> '<a %HREF% %CLASS% %CUSTOM%><img %SRC% %IMG_CLASS% %ALT% %IMG_TITLE% %SIZE% />' . $image_hover . '</a>',
			'alt'		=> $big_image['title'],
			'title'		=> $big_image['description'],
			...
			if ( is_single() ) {
				...
				if ( $ancestors ) {
					...
					foreach ( $ancestors as $ancestor ) {
						$steps .= sprintf( '<li><a href="%s">%s</a></li>', get_permalink($ancestor), presscore_substring(get_the_title($ancestor)) );
					}
				}
			}

			$steps .= '<li><a href="' . get_permalink($post->ID) . '">' . presscore_substring(get_the_title($post->ID)) . '</a></li>';
	...
	function presscore_nav_menu_list( $menu_name = '', $style = 'right' ) {
		...
		if ( ( $locations = get_nav_menu_locations() ) && isset( $locations[ $menu_name ] ) ) {
			...
			foreach ( (array) $menu_items as $key => $menu_item ) {
				...
				// target
				if ( $menu_item->target ) {
					$attr .= ' target="' . esc_attr($menu_item->target) . '"';
				}

				// title
				if ( $menu_item->attr_title ) {
					$attr .= ' title="' . esc_attr($menu_item->attr_title) . '"';
				}
				...
				// rel
				if ( $menu_item->xfn ) {
					$attr .= ' rel="' . esc_attr($menu_item->xfn) . '"';
				}
...
if ( ! function_exists( 'presscore_get_social_icons' ) ) :
	 ...
	 * @return string
	 */
	function presscore_get_social_icons( $icons ) {
		if ( !is_array($icons) ) {
			return '';
		}
		...
		foreach ( $icons as $icon_class=>$icon ) {
			$output[] = sprintf(
				...
				isset($icon['link']) ? esc_url( $icon['link'] ) : '',
				isset($icon['title']) ? esc_attr( $icon['title'] ) : '',
				esc_attr( $icon_class )
			);
		}
